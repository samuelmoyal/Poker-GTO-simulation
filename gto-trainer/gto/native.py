"""postflop-solver backend: solves one street and returns the tree in the layout `engine.py` reads.

Same contract as `solver.solve` (board, pot, stack, ip/oop range, street -> tree dict, cached on disk), but ranges
may be given per combo (`{"AhKd": 0.4}`) and the work is done by `tools/turn-labels`' `street_tree` binary.
"""
import gzip
import hashlib
import json
...
def build_request(board, pot, stack, ip_range, oop_range, street, effort=None, progress=False) -> dict:
    ...
    req = dict(
        ...
        **dict(config.NATIVE_EFFORT[street], **(effort or {})),
    )
...
def solve(board, pot, stack, ip_range, oop_range, street, cache_path=None, on_progress=None, effort=None):
    ...
    if cache_path is None:
        ...
        cache_path = os.path.join(config.NATIVE_CACHE_DIR, f"{street}_{hashlib.sha1(blob.encode()).hexdigest()}.json.gz")
    ...
    with _run_lock:
        if os.path.exists(cache_path):
            return _read_gz(cache_path)
        if not os.path.exists(config.NATIVE_BIN):
            raise SolverError(f"{config.NATIVE_BIN} manquant : cd tools/turn-labels && cargo build --release")
        req["id"] = "r"
        env = dict(os.environ, RAYON_NUM_THREADS=str(config.THREADS))
        proc = subprocess.Popen([config.NATIVE_BIN], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
